...
RUN userhome=$(eval echo ~${DEVELOPER_USERNAME}) \
 ...
 && mkdir -p /workspaces/portfolio-rails \
 && chown -R ${DEVELOPER_USERNAME}:${DEVELOPER_USERNAME} /workspaces/portfolio-rails \
 && chown -R ${DEVELOPER_USERNAME}:${DEVELOPER_USERNAME} /usr/local/bundle/*

# Add the app's "bin/" directory to PATH:
ENV PATH=/workspaces/portfolio-rails/bin:$PATH

# Set the app path as the working directory:
WORKDIR /workspaces/portfolio-rails
...
COPY --chown=${DEVELOPER_USERNAME} Gemfile* /workspaces/portfolio-rails/
...
COPY --from=testing /workspaces/portfolio-rails/ /workspaces/portfolio-rails/
...
COPY --chown=${DEVELOPER_USERNAME} . /workspaces/portfolio-rails/
...
# final destination at /workspaces/portfolio-rails:
COPY --from=builder --chown=nobody:nogroup /workspaces/portfolio-rails /workspaces/portfolio-rails

# Set the container user to 'nobody':
USER nobody

# Set the RAILS and PORT default values:
ENV HOME=/workspaces/portfolio-rails \
...
ENTRYPOINT [ "/workspaces/portfolio-rails/bin/entrypoint" ]
